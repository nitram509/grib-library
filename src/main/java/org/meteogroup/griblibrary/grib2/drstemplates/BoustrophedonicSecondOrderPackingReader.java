...
public class BoustrophedonicSecondOrderPackingReader implements DataTemplateReader {
    ...
    public DRSTemplate readTemplate(byte[] bytes, int headerOffSet) {
        ...
        boustroTemplate.setBinaryScaleFactor(asSignedInt(bytes[POSITION_BINARYSCALEFACTOR1 + headerOffSet], bytes[POSITION_BINARYSCALEFACTOR2 + headerOffSet]));
        boustroTemplate.setDecimalScaleFactor(asSignedInt(bytes[POSITION_DECIMALSCALEFACTOR1 + headerOffSet], bytes[POSITION_DECIMALSCALEFACTOR2 + headerOffSet]));
        boustroTemplate.setBitsPerValue(asInt(bytes[POSITION_BITSPERVALUE + headerOffSet]));
        boustroTemplate.setNumberOfBitsForFirstOrderValues(asInt(bytes[POSITION_WIDTHOFFIRSTORDERVALUES + headerOffSet]));
        boustroTemplate.setNumberOfGroups(asInt(bytes[POSITION_NUMBEROFGROUPS1 + headerOffSet], bytes[POSITION_NUMBEROFGROUPS2 + headerOffSet], bytes[POSITION_NUMBEROFGROUPS3 + headerOffSet], bytes[POSITION_NUMBEROFGROUPS4 + headerOffSet]));
        boustroTemplate.setNumberOfSecondOrderPackedValues(asInt(bytes[POSITION_NUMBEROFSECONDORDERPACKEDVALUES1 + headerOffSet], bytes[POSITION_NUMBEROFSECONDORDERPACKEDVALUES2 + headerOffSet], bytes[POSITION_NUMBEROFSECONDORDERPACKEDVALUES3 + headerOffSet], bytes[POSITION_NUMBEROFSECONDORDERPACKEDVALUES4 + headerOffSet]));
        boustroTemplate.setBitsForSecondaryOrderWidth(asInt(bytes[POSITION_WIDTHOFWIDTHS + headerOffSet]));
        boustroTemplate.setBitsForSecondaryOrderLength(asInt(bytes[POSITION_WITHOFLENGTHS + headerOffSet]));
        boustroTemplate.setOrderOfSPD(asInt(bytes[POSITION_ORDEROFSPD + headerOffSet]));
        boustroTemplate.setNumberOfBitsOfSPD(asInt(bytes[POSITION_NUMBEROFBITSOFSPD + headerOffSet]));
