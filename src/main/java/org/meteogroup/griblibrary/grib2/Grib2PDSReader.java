...
class Grib2PDSReader extends Grib2SectionReader {
    ...
    public Grib2PDS readPDSValues(byte[] pdsValues, int headerOffSet) throws IOException {
        ...
        pds.setNumberOfCoordinateValues(asSignedInt(pdsValues[POSITION_NROFCOORDINATEVALUES1 + headerOffSet], pdsValues[POSITION_NROFCOORDINATEVALUES2 + headerOffSet]));
        pds.setTemplateNumber(asSignedInt(pdsValues[POSITION_TEMPLATENUMBER1 + headerOffSet], pdsValues[POSITION_TEMPLATENUMBER2 + headerOffSet]));
        pds.setTemplate(this.readProductTemplate(pdsValues, headerOffSet, pds.getTemplateNumber()));
        return pds;
    }

    protected ProductTemplate readProductTemplate(byte[] pdsValues, int headerOffset, int pdsTemplateNumber) throws IOException {
